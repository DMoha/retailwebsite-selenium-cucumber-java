...
public class Actions {
	...
	public Actions Waitforpageload() {
		driverwait(Integer.parseInt(FileReaderManager.getInstance().getConfigReader().getPageloadWait()));
		return this;
	...
	public boolean verifyElementPresentinUI(ObjectLocator locator) {
		...
		try {
			driver.findElement(locator.Locator).isDisplayed();
			return true;
	...
	public void selectSize(ObjectLocator ListSelectSize, ObjectLocator SelectSize, String Size, String SizeGroup) {
		int counter=0;
		Integer rowcount = 1;
		List<WebElement> labels = FindElements(ListSelectSize);
				
		for (WebElement label : labels) {
			String labelvalue = label.getAttribute("value").trim();

			if(labelvalue.equalsIgnoreCase(Size)) {
												
				Click(SelectSize.ReplaceLocator("$", rowcount.toString(),"%", SizeGroup));
				counter =1;
				break;
			}
			++rowcount;
		...
		if(counter!=1) {
			Assert.fail("Requested size is not present");
		}
	}
